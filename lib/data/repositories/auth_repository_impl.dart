...
/// Implementación real de autenticación contra la API.
class AuthRepositoryImpl implements IAuthRepository {
  ...
  @override
  Future<(TokensAuth, Usuario)> iniciarSesion(String username, String password) async {
    ...
    try {
      ...
      if (respuesta.statusCode == 200) {
        ...
        final usuario =
            UsuarioDto.fromJson(json['user'] as Map<String, dynamic>).toDomain();

        // COMPLETAR: guarda los tokens para que la sesión persista.
  ...
  Future<TokensAuth> refrescarToken(String refresh) async {
    ...
    try {
      ...
      if (respuesta.statusCode == 200) {
        ...
        final tokens = TokensAuth(
          access: json['access'] as String,
          // El refresh ROTA: el backend manda uno nuevo. Si no viniera, conservamos el actual.
  ...
  Future<void> cerrarSesion(String refresh) async {
    ...
    try {
      ...
      // Sin red igual cerramos sesión local (abajo, en finally).
    } on TimeoutException {
      // idem
    } finally {
      // Pase lo que pase, borramos los tokens locales.
